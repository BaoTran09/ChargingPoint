...
namespace ChargingApp.Accounting
{
    public partial class frmDSChungTu : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void ShowActionMenu(int rowHandle, ButtonEdit editor = null)
        {
            ...
            // Thêm items
            if (docType == "Invoice" || docType == "Receipt" || docType == "Revenue")
            {
        ...
        // 3. ExecuteAction – GIỮ NGUYÊN CỦA ANH (HOÀN HẢO RỒI!)
        private void ExecuteAction(int rowHandle, string action)
        {
            try
            {
                ...
                string docType = row["DocumentType"]?.ToString() ?? "";
                long docId = Convert.ToInt64(row["DocumentId"]);
                string customerEmail = row.Table.Columns.Contains("CustomerEmail") ? row["CustomerEmail"]?.ToString() : "";
                // Xử lý các action cho Invoice (Chỉ có thể Gửi Mail/PDF cho Invoice)
                if (docType != "Invoice" && action != "Xem chi tiết")
                {
                    XtraMessageBox.Show($"Chức năng '{action}' chỉ hỗ trợ cho Hóa đơn.", "Thông báo");
                    gvDocument.SetRowCellValue(rowHandle, "Action", null);
                    return;
                }
                switch (action)
                {
                    case "Xem chi tiết":
                        if (docType == "Invoice")
                            ...
                            XtraMessageBox.Show("Đang phát triển form Phiếu thu...", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    case "Xuất PDF":
                        if (docType == "Invoice")
                            ...
                            XtraMessageBox.Show($"Chưa hỗ trợ xuất PDF cho loại: {docType}");
                        break;
                    ...
                    case "Gửi Email Nhắc nợ":
                        ...
                        {
                            XtraMessageBox.Show("Khách hàng không có email!", "Cảnh báo", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                            return;
                        }
                        if (docType == "Invoice")
                        {
